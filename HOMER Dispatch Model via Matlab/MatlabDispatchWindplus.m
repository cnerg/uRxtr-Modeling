...
function [avail_wind_power, simulation_state] = dispatch_wind(simulation_state, simulation_parameters)
  ...
  % simulation_state.wind_turbines(1).power_setpoint = 0; not neccessary
...
% simulation_state : object holding current state of simulation
%
function [avail_solar_ac, simulation_state] = dispatch_solar(simulation_state, simulation_parameters, inv_efficiency)
...
%%  
% step 1: dispatch wind first, and then PV
...
for option = 1:3
    ...
    if (remaining_load > eps)
        if (option == 3)
            ...
            avail_generator_power = dispatch_generator(simulation_state, simulation_parameters, remaining_load);
        end
    end
    ...
    parameters(option,ac_bus_capacity_shortage) = min( parameters(option,ac_bus_operating_capacity_requested), parameters(option,ac_bus_operating_capacity_served) );
end
 %%
